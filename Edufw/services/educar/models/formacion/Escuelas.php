...
class Escuelas extends RestModel {
    ...
    public function searchEscuelas($data) {
        try {
            ...
            $this->uri = $this->global_config['URL_SEARCH_ESCUELA'];
            return $this->callRestService();
        } catch (Exception $e) {
            ELogger::log('[Escuelas/searchEscuelas] ' . $e->getMessage(), ELoggerLevel::LEVEL_WARN);
        }
    ...
    public function getEscuelaFull($prov_id, $departamento_id, $localidad_id, $esculea_id, $sitio_id = false) {
        try {
            ...
            $this->uri = $this->global_config['URL_OBTENER_ESCUELA_FULL'];
            return $this->callRestService();
        } catch (Exception $e) {
            ELogger::log('[Escuelas/getEscuelaFull] ' . $e->getMessage(), ELoggerLevel::LEVEL_WARN);
        }
